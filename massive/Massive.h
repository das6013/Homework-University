...
private:
	...
	int n;
	int mem;
	void shiftleft(int pos);// ñäâèã âëåâî íà÷èíàÿ ñ ïîçèöèè pos+1
public:
	Arraya(int _n = 1);
	Arraya(T* b, int m);
	...
	void print();
	T& operator[](int);
	int findKey(T);// âîçâðàùàåò èíäåêñ ýëåìåíòà èëè -1, åñëè ýëåìåíòà íåò
	Arraya<T>& operator+=(T);
	Arraya<T> operator+(T);
	...
	Arraya<T>& DelPosEq(int);// âîçâðàùàåò *this
	Arraya<T>& DelPosNew(int);// âîçâðàùàåò íîâûé ìàññèâ
	bool operator==(Arraya<T>);
	bool operator!=(Arraya<T>);
	int max();//âîçâðàùàåò èíäåêñ ìàêñ.
	int min();//âîçâðàùàåò  èíäåêñ ìèí.
	void sortShella();
